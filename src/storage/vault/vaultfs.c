...
int vaultfs_root_update_snapshot_store(
    ...
    if (device == (void *)0 || plan == (const void *)0 || plan->transaction_id == 0U ||
        !vaultfs_superblock_valid(superblock) || !vaultfs_root_directory_block_valid(next_root_block) ||
        superblock->generation == UINT64_MAX ||
        ...
        next_root_block->generation != plan->next_generation ||
        next_root_block->checksum != plan->payload_checksum ||
    ...
    vaultfs_root_directory_encode(next_root_block, bytes);
