...
int horizon_input_runtime_probe(void) {
    ...
    if (!atlas_keyboard_receive_scancode(UINT8_C(0x31)) ||
        !atlas_keyboard_receive_scancode(UINT8_C(0xb1)) ||
        !atlas_keyboard_receive_scancode(UINT8_C(0x39)) ||
        !atlas_keyboard_receive_scancode(UINT8_C(0x19))) {
        return 0;
    }
    return horizon_desktop_state_initialize(&state, 3U) &&
           ...
           result.handled_event_count == 3U && result.redraw_requested == 1U && state.focused_window == 0U &&
