...
PULSE_DAWN_MADT_X86_OBJ := $(BUILD_DIR)/pulse/dawn/acpi-madt-x86.obj
...
PULSE_ATLAS_ATA_OBJ := $(BUILD_DIR)/drivers/atlas/ata-pio.obj
...
PULSE_HORIZON_INPUT_TEST := $(BUILD_DIR)/tests/horizon-input
PULSE_HORIZON_RUNTIME_TEST := $(BUILD_DIR)/tests/horizon-runtime
PULSE_KEYBOARD_TEST := $(BUILD_DIR)/tests/pulse-keyboard-bootstrap
PULSE_PCI_TEST := $(BUILD_DIR)/tests/pulse-pci-bootstrap
PULSE_ATA_TEST := $(BUILD_DIR)/tests/pulse-ata-bootstrap
...
.PHONY: all prelude pulse uefi-image bios-image check-uefi check-keyboard check-storage check-bios check-panic test test-bios test-panic clean
...
$(PULSE_DAWN_MADT_X86_OBJ): src/platform/dawn/acpi-madt-x86.c src/platform/dawn/include/dawn_acpi.h
	@mkdir -p $(dir $@)
	$(CLANG) $(PULSE_CFLAGS) -c $< -o $@
...
$(PULSE_ATLAS_ATA_OBJ): src/drivers/atlas/ata-pio.c src/drivers/atlas/include/atlas_ata.h
	@mkdir -p $(dir $@)
	$(CLANG) $(PULSE_CFLAGS) -c $< -o $@
...
$(PULSE_HORIZON_INPUT_OBJ): src/apps/horizon/input.c src/apps/horizon/include/horizon_input.h
	@mkdir -p $(dir $@)
	$(CLANG) $(PULSE_CFLAGS) -c $< -o $@

$(PULSE_HORIZON_RUNTIME_OBJ): src/apps/horizon/runtime.c src/apps/horizon/include/horizon_runtime.h
	@mkdir -p $(dir $@)
	$(CLANG) $(PULSE_CFLAGS) -c $< -o $@
...
$(PULSE_ELF): $(PULSE_ENTRY_OBJ) $(PULSE_INTERRUPTS_ENTRY_OBJ) $(PULSE_CONTEXT_ENTRY_OBJ) $(PULSE_MAIN_OBJ) $(PULSE_DAWN_ACPI_OBJ) $(PULSE_DAWN_MADT_X86_OBJ) $(PULSE_MEMORY_OBJ) $(PULSE_PAGING_OBJ) $(PULSE_INTERRUPTS_OBJ) $(PULSE_INTERRUPT_CONTROLLER_OBJ) $(PULSE_SCHEDULER_OBJ) $(PULSE_CONTEXT_OBJ) $(PULSE_TIMER_OBJ) $(PULSE_PIC_OBJ) $(PULSE_KEYS_OBJ) $(PULSE_RELAY_OBJ) $(PULSE_RELAY_CHANNEL_OBJ) $(PULSE_ORIGIN_OBJ) $(PULSE_ORIGIN_ABI_OBJ) $(PULSE_ATLAS_RAM_OBJ) $(PULSE_ATLAS_KEYBOARD_OBJ) $(PULSE_ATLAS_KEYBOARD_IRQ_OBJ) $(PULSE_ATLAS_PCI_OBJ) $(PULSE_ATLAS_ATA_OBJ) $(PULSE_VAULT_OBJ) $(PULSE_SESSION_OBJ) $(PULSE_PARCEL_OBJ) $(PULSE_PRISM_OBJ) $(PULSE_PRISM_BOOTSTRAP_OBJ) $(PULSE_CANVAS_OBJ) $(PULSE_HORIZON_OBJ) $(PULSE_HORIZON_CATALOG_OBJ) $(PULSE_HORIZON_FOCUS_OBJ) $(PULSE_HORIZON_INPUT_OBJ) $(PULSE_HORIZON_RUNTIME_OBJ) $(PULSE_PANIC_OBJ) $(PULSE_DIR)/linker/x86_64.ld
	@mkdir -p $(dir $@)
...
$(PULSE_ATA_TEST): tests/kernel/pulse_ata_bootstrap.c src/drivers/atlas/ata-pio.c src/drivers/atlas/include/atlas_ata.h
	@mkdir -p $(dir $@)
	$(HOST_CC) -std=c17 -Wall -Wextra -Wpedantic -Werror -Isrc/drivers/atlas/include \
		tests/kernel/pulse_ata_bootstrap.c src/drivers/atlas/ata-pio.c -o $@

$(DAWN_ACPI_TEST): tests/platform/dawn_acpi_rsdp.c src/platform/dawn/acpi-rsdp.c src/platform/dawn/acpi-madt-x86.c src/platform/dawn/include/dawn_acpi.h
	@mkdir -p $(dir $@)
	$(HOST_CC) -std=c17 -no-pie -Wall -Wextra -Wpedantic -Werror -Isrc/platform/dawn/include \
	tests/platform/dawn_acpi_rsdp.c src/platform/dawn/acpi-rsdp.c src/platform/dawn/acpi-madt-x86.c -o $@
...
test: check-uefi $(PULSE_MEMORY_TEST) $(PULSE_PAGING_TEST) $(PULSE_INTERRUPTS_TEST) $(PULSE_INTERRUPT_CONTROLLER_TEST) $(PULSE_SCHEDULER_TEST) $(PULSE_CONTEXT_TEST) $(PULSE_TIMER_TEST) $(PULSE_RELAY_TEST) $(PULSE_VAULT_TEST) $(PULSE_SESSION_TEST) $(PULSE_PARCEL_TEST) $(PULSE_CANVAS_TEST) $(PULSE_HORIZON_TEST) $(PULSE_HORIZON_FOCUS_TEST) $(PULSE_HORIZON_INPUT_TEST) $(PULSE_HORIZON_RUNTIME_TEST) $(PULSE_KEYBOARD_TEST) $(PULSE_PCI_TEST) $(PULSE_ATA_TEST) $(DAWN_ACPI_TEST)
	$(PULSE_MEMORY_TEST)
	$(PULSE_PAGING_TEST)
	...
	$(PULSE_HORIZON_INPUT_TEST)
	$(PULSE_HORIZON_RUNTIME_TEST)
	$(PULSE_KEYBOARD_TEST)
	$(PULSE_PCI_TEST)
	$(PULSE_ATA_TEST)
	$(DAWN_ACPI_TEST)
