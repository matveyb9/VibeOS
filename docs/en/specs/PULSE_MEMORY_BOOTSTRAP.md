...
The first Pulse memory layer is intentionally a bootstrap, not a finished general allocator. It accepts the UEFI memory map handed over in Dawn Context, selects the first usable `EfiConventionalMemory` descriptor, and allocates contiguous 4 KiB physical frames from that one region. The first boot validates the handoff by allocating two adjacent frames.
...
| Field | Meaning |
|---|---|
| `selected_region_base` | Start of the first selected conventional-memory region. |
| `selected_region_limit` | Exclusive end of that region. |
| `next_free_frame` | Next frame that the bootstrap allocator will hand out. |
| `usable_page_count` | Count of 4 KiB frames in the selected region. |

## Explicit limits

The bootstrap does not coalesce regions, reserve pages already consumed by later Pulse subsystems, support freeing, or create virtual mappings. It is a narrow prerequisite for the x86_64 page-table and interrupt stages. Future Pulse memory management will replace it with an ownership-aware physical allocator while preserving the Dawn Context ABI boundary.
