...
| ELF64 load plan | Caller-owned immutable summary formed only when the nonzero entry address falls inside the already validated declared `PT_LOAD` virtual span; it contains entry address, span, and segment count, not memory operations |
...
`parcel_elf64_load_plan_form` is an immutable handoff-preparation boundary. It combines only caller-owned header and program-header summaries after verifying a nonzero entry address within the declared virtual range. Its result is descriptive future-loader metadata, not a command: it has no media reference, physical frame, page-table mutation, address-space identity, permission grant, capability, process/thread record, callback, or execution state. The test suite rejects an entry at the upper bound of the declared range.
