...
| Native app catalog | Three static descriptors bind logical cards to Prompt, Cue, and Vector IDs; an explicit Enter selection resolves a descriptor only for a bounded Parcel-facing request |
...
`horizon_build_desktop_scene_for_state` accepts a validated three-window focus state and positions the retained indicator at its focused card. Static catalog descriptors bind those cards, and their rendered labels, to Prompt, Cue, and Vector. When selection exists, the selected descriptor can be resolved by its logical window and the scene adds one amber strip inside the selected card's lower edge; the original bootstrap builder creates no selection. The Horizon input adapter is the deliberate cross-subsystem boundary: it accepts an already-normalized `ATLAS_KEY_EVENT`, maps recognised Tab, Shift+Tab, Left/Right Arrow, and Enter press events into an existing focus action, and reports whether the event was handled. Its bounded pump is the only consumer of Atlas events in this layer: it never reads a device register, does not allocate, and stops at a caller-selected budget no greater than eight. A runtime step reports a selected descriptor only for an explicit handled Enter selection; Pulse may pass its native ID to Parcel, which creates a canonical request record and admits it only if a matching installed manifest exists. The caller-owned desktop runtime uses the catalog's exact count when it initializes the framebuffer copy, focus state, initial render, and one pump-to-redraw step while deliberately leaving waiting and IRQ delivery to Pulse. The dedicated x86_64 keyboard QEMU profile performs a controlled two-event session: IRQ1 queues Right Arrow and the runtime redraws focus, then a second IRQ1 queues Enter and the same runtime selects the persisted focused card, forms the Parcel request, redraws its amber strip, and exits. Host tests verify state bounds, catalog mapping, selected-descriptor resolution, forward and backward navigation, selection, adapter mapping, partial and complete draining, redraw reporting, selection intent, Parcel admission, sequential runtime lifecycle, scene geometry, selection pixels, and rendering order.

> The catalog does not contain executable images, launch callbacks, package identities, or permission grants. The request record is not a launch: it carries no image location, executable bytes, callback, process identity, ownership, or rights. The keyboard QEMU profile is a controlled two-event proof, not a general live desktop loop. The runtime does not wait, configure interrupts, schedule itself, own device queues, operate windows, launch processes, or enforce permissions. Future work will schedule it outside the test profile, coalesce Canvas/Prism redraws, add pointer queues, use Keys for window ownership, admit actual packages through Parcel, and integrate the actual applications named in the project architecture.
