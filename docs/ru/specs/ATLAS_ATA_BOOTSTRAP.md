...
**Статус:** Реализован как bounded, read-only ATA `IDENTIFY DEVICE` probe для воспроизводимого x86_64 QEMU PC IDE profile.
...
| Verification | Deterministic injected-transport host test и отдельный UEFI/OVMF QEMU PC IDE probe с temporary 1 MiB raw disk |

Host fixture проверяет LBA28 response, предпочитает supported nonzero LBA48 capacity и отклоняет floating bus, ATA error status, invalid device selection и overflowing command-base range. QEMU check запускает отдельный image `PULSE_PROBE_storage` на QEMU machine `pc`, подключает empty temporary IDE disk как primary device zero, наблюдает `ATLAS: ATA identify verified` и проверяет standard debug-exit status. Документация QEMU описывает PC machine как предоставляющую PCI IDE interface с hard-disk support.[3]

> Это **physical controller identity/capacity probe**, а не general disk driver. Он не выдаёт sector read/write command, не использует DMA, не reset bus, не изменяет interrupt policy, не configures PCI, не parse partition, не bind Atlas block device, не exposes ATA sector interface и не connect physical device к VaultFS.

Probe не устанавливает durability, media ownership, storage trust, hot-plug behavior, controller discovery за пределами fixed QEMU legacy range, cache behavior, error recovery или compatibility с arbitrary ATA/SATA/AHCI hardware. Любой последующий read path должен получить собственный bounded addressing, buffer, failure и device-selection contract; writable persistence path дополнительно требует explicit physical flush и crash-consistency model.
