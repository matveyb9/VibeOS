...
VaultFS теперь может выполнить следующий bounded transition: снова load persisted `PREPARED` record, load и validate уже записанный alternate root record, а затем write newly sealed `COMMITTED` journal record обратно в тот же явно отдельный journal block. Stored root должен validate с planned next generation и exact planned checksum; journal должен всё ещё быть `PREPARED` и совпадать по transaction, alternate target и checksum. Тесты показывают, что malformed journal bytes либо valid record с wrong target не могут быть committed, а commit journal не promote old superblock generation 7 и не делает recovery decision accepting root generation 8 под ним.

Этот transition намеренно не является recovery replay или atomic persistence claim. Он не write superblock, не определяет, какую superblock copy promote, не изменяет root reference, не предоставляет persistence barrier, не recovery torn write и не mutate media during recovery. Semantics physical device и отдельно проверяемая superblock-promotion operation остаются последующей работой.

VaultFS теперь может form и store одну next-generation redundant superblock copy после повторной загрузки matching persisted `COMMITTED` journal и повторной validation alternate root snapshot. Caller явно выбирает destination superblock block; он должен быть distinct от journal и обоих root slot. Successor сохраняет current slot state и fixed primary/backup root reference, advance generation до next generation из plan и advance journal sequence на единицу. Второй вызов может независимо write peer copy. Host coverage доказывает, что first promoted copy делает alternate snapshot generation 8 current через existing latest-superblock и dual-root selection, а corruption second copy всё ещё делает fallback к valid first promoted copy.

Это по-прежнему ordered Atlas RAM/block operation, а не physical crash-consistency guarantee. Он не предоставляет write ordering или flush contract, recovery torn write, proof, что обе копии достигли non-volatile media, replay, journal clearing или transaction allocator. Следующая работа должна определить recovery policy для committed record вместе с partially либо fully promoted redundant superblock до заявления complete root-update protocol.

VaultFS теперь предоставляет это decision input как bounded **read-only recovery inspection**. Он load highest valid superblock, journal record из явно отдельного block и journal-selected alternate root record, а затем validate exact target и checksum binding. Inspection сообщает одно из трёх observable state: `PREPARED`, когда sealed prepared record описывает next root generation; `COMMITTED_UNPROMOTED`, когда sealed committed record описывает эту next generation; либо `COMMITTED_PROMOTED`, когда generation selected superblock уже равна generation committed root. Он reject mismatched root, journal/root overlap, invalid media и invalid state/generation combination до любой записи.

Inspection намеренно отвечает только на то, что показывает bounded RAM/block media state. Он не discard prepared record, не replay committed record, не repair или promote superblock copy, не выбирает application-visible filesystem state, не clear journal, не apply device flush и не гарантирует recovery после потери питания. Эти действия требуют отдельного последующего recovery-policy и physical-driver contract.
