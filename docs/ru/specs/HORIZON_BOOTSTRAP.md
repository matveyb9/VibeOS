...
| Native app catalog | Три static descriptor связывают logical card с Prompt, Cue и Vector ID; explicit Enter selection resolve descriptor только для bounded Parcel-facing request |
...
`horizon_build_desktop_scene_for_state` принимает valid focus state из трёх window и размещает retained indicator у focused card. Static catalog descriptor связывают эти card и их rendered label с Prompt, Cue и Vector. Когда selection существует, selected descriptor можно resolve по logical window, а scene добавляет один amber strip у нижнего края selected card; исходный bootstrap builder не создаёт selection. Horizon input adapter — преднамеренная cross-subsystem boundary: он принимает уже normalized `ATLAS_KEY_EVENT`, отображает recognised Tab, Shift+Tab, Left/Right Arrow и Enter press event на существующий focus action и сообщает, был ли event обработан. Его bounded pump — единственный consumer Atlas event на этом layer: он никогда не читает device register, не выделяет память и останавливается на caller-selected budget не больше восьми. Runtime step сообщает selected descriptor только для explicit handled Enter selection; Pulse может передать его native ID в Parcel, который создаёт canonical request record и допускает его только при существующем matching installed manifest. Caller-owned desktop runtime использует exact count catalog при инициализации framebuffer copy, focus state, initial render и одного pump-to-redraw step, намеренно оставляя wait и IRQ delivery Pulse. Выделенный x86_64 keyboard QEMU profile выполняет controlled two-event session: IRQ1 помещает Right Arrow в queue, runtime перерисовывает focus, затем второй IRQ1 помещает Enter и тот же runtime выбирает persisted focused card, формирует Parcel request, перерисовывает amber strip и завершает proof. Host-тесты проверяют state bound, catalog mapping, selected-descriptor resolution, forward и backward navigation, selection, adapter mapping, partial и complete draining, redraw reporting, selection intent, Parcel admission, sequential runtime lifecycle, scene geometry, selection pixel и rendering order.

> Catalog не содержит executable image, launch callback, package identity или permission grant. Request record не является launch: он не содержит image location, executable byte, callback, process identity, ownership или right. Keyboard QEMU profile — это controlled proof двух event, а не общий live desktop loop. Runtime не ожидает, не настраивает interrupt, не schedule сам себя, не владеет device queue, не управляет window, не запускает process и не применяет permission. Будущая работа будет schedule его вне test profile, coalesce Canvas/Prism redraw, добавлять pointer queue, использовать Keys для window ownership, допускать actual package через Parcel и интегрировать настоящие приложения из project architecture.
