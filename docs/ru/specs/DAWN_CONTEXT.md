...
Pulse выбирает nonzero XSDT pointer из RSDP revision 2 или новее, а иначе использует RSDT pointer. Он проверяет expected signature выбранной root table, minimum header 36 byte, bounded total length, делимость на entry width и checksum всей table, после чего извлекает её physical address, byte length, kind и entry count как transient metadata.[1] Отдельная injected-reader operation может скопировать не более 64 RSDT/XSDT entry, сохранить physical address, signature, declared length, revision и checksum state каждого child common header, а также сообщить omitted entry. Когда присутствует validated `APIC` child, отдельная read-only operation проверяет full-table checksum, сохраняет fixed MADT metadata размером 44 bytes и не более 64 type-and-length entry header; zero-length, truncated, overflowing, wrong-signature и bad-checksum input отклоняются. Отдельный x86 decoder читает только fixed-size field Local APIC (type 0), I/O APIC (type 1) и Interrupt Source Override (type 2) в bounded metadata record; нераспознанные entry остаются generic. Текущий x86_64 Pulse reader намеренно принимает только physical address ниже 4 GiB, поскольку early identity paging имеет ту же границу.

> MADT work наблюдает только fixed metadata и generic entry header; он не интерпретирует descriptor payload. Child inventory — только metadata, а не permission на доступ к описанному device. Он не выполняет AML, не включает ACPI, не программирует local APIC или I/O APIC, не запускает processor, не создаёт SMP state, не управляет power state, не назначает PCI resource, не программирует interrupt routing и не заявляет совместимость с реальным hardware.
