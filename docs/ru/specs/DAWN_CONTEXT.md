...
Pulse выбирает nonzero XSDT pointer из RSDP revision 2 или новее, а иначе использует RSDT pointer. Он проверяет expected signature выбранной root table, minimum header 36 byte, bounded total length, делимость на entry width и checksum всей table, после чего извлекает её physical address, byte length, kind и entry count как transient metadata.[1] Отдельная injected-reader operation может скопировать не более 64 RSDT/XSDT entry, сохранить physical address, signature, declared length, revision и checksum state каждого child common header, а также сообщить omitted entry. Текущий x86_64 Pulse reader намеренно принимает только physical address ниже 4 GiB, поскольку early identity paging имеет ту же границу.

> Этот milestone **не** интерпретирует child-table payload RSDT/XSDT или какой-либо другой ACPI table. Child inventory — только metadata, а не permission на доступ к описанному device. Он не выполняет AML, не включает ACPI, не перечисляет CPU или interrupt, не управляет power state, не назначает PCI resource, не программирует interrupt routing и не заявляет совместимость с реальным hardware.
