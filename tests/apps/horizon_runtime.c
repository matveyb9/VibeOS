...
int main(void) {
    ...
    if (!expect(!horizon_desktop_runtime_initialize(&framebuffer, (void *)0), "null runtime is rejected") ||
        ...
        !expect(atlas_keyboard_receive_scancode(UINT8_C(0x31)) && horizon_desktop_runtime_step(&runtime, 1U, &result) &&
                ...
                "N event advances focus and redraws retained indicator") ||
