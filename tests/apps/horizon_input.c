...
int main(void) {
    ...
    if (!expect(horizon_desktop_state_initialize(&state, 3U), "state initializes") ||
        ...
        !expect(!horizon_input_apply_event(&state, &(ATLAS_KEY_EVENT){UINT8_C(0x0f), 1U, '\0', ATLAS_KEY_TAB, ATLAS_KEY_NONE}, (void *)0),
                "null handled output is rejected") ||
        !expect(horizon_input_apply_event(&state, &(ATLAS_KEY_EVENT){UINT8_C(0x0f), 1U, '\0', ATLAS_KEY_TAB, ATLAS_KEY_NONE}, &handled) &&
                    handled == 1U && state.focused_window == 1U,
                "Tab press advances focus") ||
        !expect(horizon_input_apply_event(&state, &(ATLAS_KEY_EVENT){UINT8_C(0x0f), 0U, '\0', ATLAS_KEY_TAB, ATLAS_KEY_NONE}, &handled) &&
                    handled == 0U && state.focused_window == 1U,
                "key release is ignored") ||
        !expect(horizon_input_apply_event(
                    &state,
                    &(ATLAS_KEY_EVENT){UINT8_C(0x0f), 1U, '\0', ATLAS_KEY_TAB, ATLAS_KEY_MODIFIER_SHIFT},
                    &handled) &&
                    handled == 1U && state.focused_window == 0U,
                "Shift+Tab press moves focus backward") ||
